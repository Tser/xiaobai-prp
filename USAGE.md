...
### 简介
PRP (Python Registry Provider) 是一个用于管理 Python 包索引源的工具，类似于 npm 的 nrm。它可以帮助您轻松切换不同的 PyPI 镜像源。

### 基本用法

安装 PRP 后，您可以使用它在不同的 Python 包索引之间切换：

```bash
# 列出所有可用的包索引源
prp ls

# 切换到特定的包索引源（例如 TUNA 镜像）
prp use tuna
...
一旦使用 PRP 切换到某个包索引源，您可以正常使用 pip。如果您想临时使用特定索引，可以检查当前包索引源：
...
PRP 将其配置存储在 `~/.prp/config.json` 中。这包括所有可用的包索引源和当前活动的包索引源。

### 可用的包索引源

默认情况下，PRP 包含这些流行的包索引源：
- `pypi`: 官方 PyPI 仓库
- `tuna`: 清华大学镜像
- `aliyun`: 阿里云镜像
- `douban`: 豆瓣镜像
...
您可以通过直接编辑 `~/.prp/config.json` 或使用 `prp add` 和 `prp del` 命令来自定义包索引源。

### 故障排除

如果遇到问题：
1. 确保您已安装最新版本的 PRP
2. 检查您的互联网连接是否能正常访问所选的包索引源
3. 验证包索引源 URL 是否可访问
